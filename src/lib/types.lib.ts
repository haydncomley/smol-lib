...
export type ComponentOptions = {
    ...
    arg: <T, K>(key: string, options: {transformer?: (value: T) => K, initial?: K}) => ComponentVariable<typeof options['transformer'] extends undefined ? T : K>,
    store: <T, K>(key: string, options: {transformer?: (value: T) => K, initial?: K}) => ComponentVariable<typeof options['transformer'] extends undefined ? T : K>,
    action: <T extends Event>(callback: (event: T extends never ? Event : T) => void) => ComponentEvent<{ type: 'BIND', event: (event: T) => void }>,
    on: {
        create: () => void
    },
    element: ShadowRoot,
